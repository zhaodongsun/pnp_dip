...
def run(f_name, specific_result_dir, noise_sigma, num_iter, rho, sigma_0, L, shrinkage_param):
    ...
    for t in range(num_iter):
        # for x
        with torch.no_grad():
            x = linf_prox_x_b(Gz.detach() - scaled_lambda_, b, shrinkage_param / rho)
        ...
        prior_loss = torch.max(torch.abs(Gz - b)) # TODO: consider using x - b

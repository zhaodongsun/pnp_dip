...
def run(f_name, specific_result_dir, noise_sigma, num_iter, GD_lr, shrinkage_param):
    img = imread(f_name)
    if img.dtype == 'uint8':
    ...
    b = x_true.reshape(-1,)
    b = torch.clamp(b + noise_sigma * (2 * torch.rand(b.shape) - 1).type(dtype), 0, 1)
    if num_channels == 3:
        imsave(specific_result_dir+'corrupted.png', b.reshape(1, num_channels, 128, 128)[0].permute((1,2,0)).cpu().numpy())
    else:
        imsave(specific_result_dir + 'corrupted.png', b.reshape(1, num_channels, 128, 128)[0, 0].cpu().numpy(), cmap='gray')
    ...
    for t in range(num_iter):
        ...
        prior_loss = torch.max(torch.abs(x.reshape(-1) - b))
        total_loss = fidelity_loss + shrinkage_param * prior_loss
        opt.zero_grad()
        total_loss.backward()
        opt.step()
        ...
        record["prior_loss"].append(prior_loss.item())
        record["cpu_time"].append(time.time())
        record["total_loss"].append(total_loss.item())
...
dataset_dir = '../data/'
results_dir = '../data/DIP_linf_subgrad/'

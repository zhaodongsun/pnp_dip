...
def run(f_name, specific_result_dir, noise_sigma, num_iter, num_ratio, GD_lr):
    img = imread(f_name)
    if img.dtype == 'uint8':
    ...
    if num_channels == 3:
        imsave(specific_result_dir+'corrupted.png', At(b).reshape(1, num_channels, 128, 128)[0].permute((1,2,0)).cpu().numpy())
    else:
        imsave(specific_result_dir + 'corrupted.png', At(b).reshape(1, num_channels, 128, 128)[0, 0].cpu().numpy(), cmap='gray')
...
dataset_dir = '../../data/'
results_dir = '../../data/results/DIP_tv_subgrad/'

...
def run(f_name, specific_result_dir, noise_sigma, num_iter, GD_lr):
    ...
    if num_channels == 3:
        imsave(specific_result_dir+'corrupted.png', b_clipped.reshape(1, num_channels, 512, 512)[0].permute((1,2,0)).cpu().numpy())
    else:
        imsave(specific_result_dir + 'corrupted.png', b_clipped.reshape(1, num_channels, 512, 512)[0, 0].cpu().numpy(), cmap='gray')
...
results_dir = '../../data_visual/results/DIP_uniform/'
